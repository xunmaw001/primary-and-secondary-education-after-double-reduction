    ...
    public R info(@PathVariable("id") Long id, HttpServletRequest request){
        ...
        if(huodong !=null){
            ...
            BeanUtils.copyProperties( huodong , view );//把实体数据重构到view中
                ...
                //级联表
                LaoshiEntity laoshi = laoshiService.selectById(huodong.getLaoshiId());
                if(laoshi != null){
    ...
    */
    ...
    public R save(@RequestBody HuodongEntity huodong, HttpServletRequest request){
        ...
        if(false)
            ...
            huodong.setLaoshiId(Integer.valueOf(String.valueOf(request.getSession().getAttribute("userId"))));

        Wrapper<HuodongEntity> queryWrapper = new EntityWrapper<HuodongEntity>()
            .eq("laoshi_id", huodong.getLaoshiId())
                ...
                if(!".xls".equals(suffix)){
                    ...
                    if(!file.exists()){
                        ...
                        for(List<String> data:dataList){
                            //循环
                            HuodongEntity huodongEntity = new HuodongEntity();
//                            huodongEntity.setLaoshiId(Integer.valueOf(data.get(0)));   //老师 要改的
//                            huodongEntity.setHuodongName(data.get(0));                    //线下活动名称 要改的
//                            huodongEntity.setHuodongPhoto("");//照片
//                            huodongEntity.setHuodongTypes(Integer.valueOf(data.get(0)));   //线下活动类型 要改的
//                            huodongEntity.setHuodongClicknum(Integer.valueOf(data.get(0)));   //点击次数 要改的
